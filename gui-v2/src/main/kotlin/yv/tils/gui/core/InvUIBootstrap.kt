...
/**
 * One-time setup required before any `gui-v2`/InvUI window is opened:
 ...
 * anyone integrating with `gui-v2` who might otherwise forget the
 ...
 * lifecycle (`onLoad`/`enablePlugin`) ever being invoked: when `gui-v2` is
 * pulled in only as a *transitive* Maven dependency of another dynamically-
 * fetched module (e.g. `multiMine` depending on `gui-v2`) without being
 ...
 * enabled there get their lifecycle methods called. `gui-v2` is a library

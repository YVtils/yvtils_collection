...
object DataClassConfigGui {
    ...
    private fun <R : Any> buildFieldItem(player: Player, field: FieldDescriptor, root: RootState<R>, screen: Screen): Item {
        return Item.builder()
            ...
            .setItemProvider { buildFieldItemBuilder(resolveIcon(field, screen.instance), field, screen.instance, player) }
    ...
    private fun <R : Any> openTextInput(player: Player, field: FieldDescriptor, root: RootState<R>, screen: Screen) {
        ...
        AnvilWindow.builder()
            ...
            .setFallbackWindow(buildLevelWindow(player, root, screen.instance, screen.title, screen.parent).build(player))
            .open(player)
    }
    ...
    private fun <R : Any> buildListEditorWindow(
        ...
        val entryItems = items.map { name -> buildListEntryItem(player, name, field.isMaterialNameList, items, ::rebuild) }
    ...
    private fun <R : Any> openAddItemInput(
        ...
        val confirmItem = Item.builder()
            ...
            .addClickHandler { _, _ ->
                val name = if (field.isMaterialNameList) pending.trim().uppercase().replace(' ', '_') else pending.trim()
                if (name.isNotBlank()) {
